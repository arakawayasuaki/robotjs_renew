...
static KeyNames key_names[] =
{
	...
	{ "convert",          K_CONVERT},
	{ "hiragana",         K_HIRAGANA},
	{ "nonconvert",       K_NONCONVERT},
	{ "hankaku",          K_HANKAKU},
	{ "zenkaku",          K_ZENKAKU},
	{ "alphanumeric",     K_ALPHANUMERIC},
	{ "katakana",         K_KATAKANA},
	{ "romaji",           K_ROMAJI},
	{ "kanamode",         K_KANAMODE},
	{ "kanjimode",        K_KANJIMODE},
	#endif
...
NAN_METHOD(getCurrentDesktop)
{
	std::string display = getCurrentDesktopI();
	info.GetReturnValue().Set(Nan::New<String>(display.c_str()).ToLocalChecked());
}

NAN_METHOD(getActiveDesktop)
{
	std::string display = getActiveDesktopI();
	info.GetReturnValue().Set(Nan::New<String>(display.c_str()).ToLocalChecked());

}

NAN_METHOD(setThreadActiveDesktop)
{
	setThreadActiveDesktopI();
	info.GetReturnValue().Set(Nan::New(1));
}
...
NAN_MODULE_INIT(InitAll)
{
	...
	Nan::Set(target, Nan::New("getCurrentDesktop").ToLocalChecked(),
		Nan::GetFunction(Nan::New<FunctionTemplate>(getCurrentDesktop)).ToLocalChecked());

	Nan::Set(target, Nan::New("getActiveDesktop").ToLocalChecked(),
		Nan::GetFunction(Nan::New<FunctionTemplate>(getActiveDesktop)).ToLocalChecked());

	Nan::Set(target, Nan::New("setThreadActiveDesktop").ToLocalChecked(),
		Nan::GetFunction(Nan::New<FunctionTemplate>(setThreadActiveDesktop)).ToLocalChecked());
